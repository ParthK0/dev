...
export default function DashboardPage() {
  ...
  // Stats
  const currentStreak = logs.length > 0 ? 12 : 0; // Dummy value based on logs
  const totalHours = logs.reduce((sum, log) => sum + log.hours, 0) + 148; // Dummy base
  const topLanguage = "TypeScript";
  const thisWeekHours = logs.slice(0, 5).reduce((sum, log) => sum + log.hours, 0) + 9.5;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      setLogs([newLog, ...logs].slice(0, 10)); // keep max 10
      setIsSubmitting(false);
      setShowSuccess(true);
      
      // Reset form (except some defaults maybe)
      setHours("");
      setProject("");
      setNotes("");
      setMood("");
    ...
    <div className="min-h-screen flex flex-col font-sans bg-background">
      ...
      <nav className="flex items-center justify-between px-6 py-4 border-b border-border bg-card">
        ...
        <div className="flex items-center gap-4 text-sm font-medium">
          ...
          <Link href="/" className="text-muted-foreground hover:text-foreground transition-colors ml-4">
            Sign Out
          </Link>
        </div>
      </nav>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-card border border-border rounded-[10px] p-5 shadow-sm">
            ...
            <div className="text-3xl font-bold text-foreground flex items-center gap-2">
              <AnimatedCounter value={currentStreak} /> days
              {currentStreak > 0 && (
                <motion.span
          ...
          <div className="bg-card border border-border rounded-[10px] p-5 shadow-sm">
            ...
            <div className="text-3xl font-bold text-foreground">
              <AnimatedCounter value={Math.floor(totalHours)} />h
            </div>
          </div>
          <div className="bg-card border border-border rounded-[10px] p-5 shadow-sm">
            <div className="text-sm text-muted-foreground mb-2 flex items-center gap-2">
